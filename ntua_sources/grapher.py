...
bandwidthEthernet = 1*1024*1024*1024
...
def draw_continuum(filename: string, color_map, graph, mode=None):
    ...
    if mode is None:
        nx.draw(graph, node_size=120, node_color=color_map, linewidths=0.01, font_size=6, font_weight='bold', with_labels=True)
    else:
        pos = drawLayouts[mode](graph)
        drawFuncs[mode](graph, node_size=120, node_color=color_map, linewidths=0.01, font_size=6, font_weight='bold', with_labels=True)
...
def create_continuum(size=6):
    # Graph creation
    # G = nx.star_graph(NUM_EDGES)
    # G2 = nx.barabasi_albert_graph(NUM_EDGES, NUM_LINK_PER_EDGE)
    # G2 = nx.generators.classic.balanced_tree(8, 2)
    G2 = nx.generators.classic.binomial_tree(size)

    NODES = G2.number_of_nodes()
    NUM_NODES_ACTIVATED = floor(NODES / 4)
    NUM_NODES_WITH_IMAGE = floor(NODES / 16)
